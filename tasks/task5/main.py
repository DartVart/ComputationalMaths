...
QF_TYPES = {"gauss": "Гаусса", "meler": "Мелера"}
    ...
    for node_count in sorted(node_counts):
        ...
        if need_to_show_real_integral:
            real_integral_value = float(
                integrate(sympy_function * meler_qf.weight_function, ("x", line_segment.left, line_segment.right))
            )
        display_result(
            approx_integral,
            nodes,
...
def main():
    ...
    sympy_function = custom_parse_expr(input_sympy_function(st, "sin(x) / x", key=get_new_key(st)))
    ...
    if qf_type == QF_TYPES["meler"]:
        ...
        line_segment = input_line_segment(
            (col2, col3), (get_new_key(st), get_new_key(st)), left_initial=0.0, right_initial=3.0
        )
    ...
    node_counts = nodes_count_col2.multiselect(
        "Количество узлов", list(range(1, max_node_count + 1)), default=3 if max_node_count >= 3 else max_node_count
    )
